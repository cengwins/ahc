...
class Node(ComponentModel):
    ...
    Shamirs_tp = ShamirsThreePass(512)

    def begin_messaging(self, msg):

        global are_step_prints_removed

        msg_encoded = Node.Shamirs_tp.encodeMessage(msg)
        msg_encrypted = Node.Shamirs_tp.encrypt(msg_encoded, self.encryption_key)

        if self.componentinstancenumber == 0:
            gmh = GenericMessageHeader(MessageTypes.SS1,0,1)
        else:
            gmh = GenericMessageHeader(MessageTypes.SS1,1,0)
        ...
        if not are_step_prints_removed:
            with s_print_lock:
                ...
                sending Step1 message with content: \"{msg}\"")
    ...
    def on_message_from_bottom(self, eventobj: Event):
        ...
        if eventobj.eventcontent.getHeader().getMessageType() == MessageTypes.SS1:

            if not are_step_prints_removed:
                with s_print_lock:
                    print(f"I am {self.componentname}.{self.componentinstancenumber} \
                     just received Step1 message and sending Step2 message")

            msg_encrypted = Node.Shamirs_tp.encrypt(eventobj.eventcontent.getPayload().getMessagePayload(), \
                                                    self.encryption_key)

            gmh = GenericMessageHeader(MessageTypes.SS2, self.componentinstancenumber,
                                       eventobj.eventcontent.getHeader().getMessageFrom())
            gmp = GenericMessagePayload(msg_encrypted)
            gm = GenericMessage(gmh, gmp)
        ...
        elif eventobj.eventcontent.getHeader().getMessageType() == MessageTypes.SS2:

            if not are_step_prints_removed:
                with s_print_lock:
                    print(f"I am {self.componentname}.{self.componentinstancenumber} \
                     just received Step2 message and sending Step3 message")

            msg_decrypted = Node.Shamirs_tp.decrypt(eventobj.eventcontent.getPayload().getMessagePayload(), \
                                                    self.decryption_key)

            gmh = GenericMessageHeader(MessageTypes.SS3, self.componentinstancenumber,
                                       eventobj.eventcontent.getHeader().getMessageFrom())
            gmp = GenericMessagePayload(msg_decrypted)
            gm = GenericMessage(gmh,gmp)
            ...
            plain_msg = Node.Shamirs_tp.decodeMessage(msg_decrypted)

            if not are_step_prints_removed:
                with s_print_lock:
                    print(f"I am {self.componentname}.{self.componentinstancenumber} \
                     just received Step3 message with content: \"{plain_msg}\"")
    ...
    def __init__(self, componentname, componentid):
        ...
        self.encryption_key = Node.Shamirs_tp.generateEncryptionKey()
        self.decryption_key = Node.Shamirs_tp.generateDecryptionKey(self.encryption_key)

        self.receive_counter = 0

    def on_init(self, eventobj: Event):

        if self.componentinstancenumber == 0: # Alice

            global are_step_prints_removed, start_time

            start_time = time.time()

            msg = "A message from Sender Node Alice to Channel"
