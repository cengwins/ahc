...
def main():
    ...
    num_nodes = 2
    topo.construct_winslab_topology_without_channels(num_nodes, AdHocNode, 0)
    ...
    for i in range(num_nodes):
        topo.nodes[i].phy.trigger_event(Event(None, EventTypes.INIT, ""))
    ...
    while(True):
        for i in range(num_nodes):
            frame = topo.nodes[i].appl.frame
            if frame is not None:
            #out.write(frame)
                f = cv2.resize(frame, (640,480))
                cv2.imshow('frame'+str(i), f)
                c = cv2.waitKey(1)
                if c & 0xFF == ord('q'):
                    break
