...
class Coordinator(ComponentModel):
            ...
            if sender == "ApplicationComponent" and message_type == "INITIATE":
                if self.componentinstancenumber == 0:
                    message_header = GenericMessageHeader("INITIATEBFSCONSTRUCTION",
                                                          ...
                                                          "FredericksonAlgorithmAdvancedComponent-" + str(self.componentinstancenumber))
                ...
                if message_type == "APPRESPONSE" or len(self.RoutingTable) > 0:
                    ...
                    temp = []
                    for k in range(len(path_to_follow)):
                        temp.append(path_to_follow[k])
                        if path_to_follow[k] == dest:
                            break
                    path_to_follow = temp
    ...
    def on_message_from_peer(self, eventobj: Event):
        ...
        message_type = eventobj.eventcontent.header.messagetype
        message = eventobj.eventcontent.payload
        # print(f"*****Coordinator {sender} {messageto} {message_type} {message}")
        if messageto == Coordinator.__name__:
            if sender == "FredericksonAlgorithmAdvancedComponent" and message_type == "BFSTREECONSTRUCTED":
    ...
    def on_message_from_bottom(self, eventobj: Event):
        ...
        message_type = eventobj.eventcontent.header.messagetype
        message = eventobj.eventcontent.payload
        # print(f"Coordinator receives message from bottom {messageto}")
        if messageto == Coordinator.__name__:
