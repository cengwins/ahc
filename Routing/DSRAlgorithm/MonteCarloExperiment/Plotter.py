...
def plot_graph(results, choice, y_label, y_axis_lim, title, save_path):
    for node_number in results:
        ...
        for prob in results[node_number]:
            ...
            for (req_time, req_msg, route_len) in results[node_number][prob]:
                if 0 == choice:
...
def get_results():
    ...
    for file in os.listdir(directory):
        ...
        request_message_time = result_json["time"]["request"]
        request_message_count = result_json["request_message_count"]
        route_length = result_json["route"][1]["route_node_count"] - 1

        try:
            results[total_node_number][probability_of_edge].append([request_message_time,
...
plot_graph(results=result,
           choice=0,
           y_label="Request Msg Time in us",
           y_axis_lim=[0, 40000],
           title="Request Msg Time vs Prob. of Edge for Node",
           save_path="Graphs\\Request_Msg_Time_Bar_Graphs.png")
...
plot_graph(results=result,
           choice=2,
           y_label="Route Hop Count",
           y_axis_lim=[0, 10],
           title="Route Hop Count vs Prob. of Edge for Node",
           save_path="Graphs\\Route_Hop_Count_Bar_Graphs.png")
