...
class OLSRComponent(ComponentModel):
    ...
    def on_time(self, current_time, _delta):
        if (current_time - self.last_hello).total_seconds() > 0.15:
            self.last_hello = current_time
            self.broadcast_hello()
        if (current_time - self.last_tc).total_seconds() > 0.1:
            self.last_tc = current_time
            self.broadcast_tc()
...
class HOLSRLayerComponent(OLSRComponent):
    def __init__(self, component_name, component_id, layer_id, is_head):
        ...
        self.decorate_with_scope(lambda p: p['cluster_head'] == self.cluster_head and p['layer'] == self.layer_id)
        self.message_handlers[HOLSRMessageTypes.CIA] = lambda payload:\
            self.on_cia(payload['address'], payload['layer'], payload['cluster_head'], payload['distance'])
        self.message_handlers[HOLSRMessageTypes.HTC] = lambda payload:\
            self.on_htc(payload['layer'], payload['address'], payload['members'])

    def decorate_with_scope(self, val_fn):
        def ft(p, f):
            if val_fn(p):
                f(p)
        for k, v in self.message_handlers.items():
            self.message_handlers[k] = lambda p: ft(p, v)
    ...
    def broadcast_msg(self, msg_type, payload):
        ...
        if 'layer' not in payload.messagepayload:
            payload.messagepayload['layer'] = self.layer_id
    ...
    def on_cia(self, sender_address, layer, sender_head, sender_distance):
        ...
        if layer <= self.layer_id:
            ...
            for i in range(len(self.cluster_member_table[sender_head])):
                (member_addr, member_hop) = self.cluster_member_table[sender_head]
    ...
    def on_htc(self, _layer, cluster_head, cluster_member_list):
...
class HOLSRComponent(ComponentModel):
    def __init__(self, component_name, component_id, layers_set=None):
        super().__init__(component_name, component_id)
        ...
        for i in range(3):
            if i in layers_set:
                self.layers.append(HOLSRLayerComponent(
                    component_name, component_id, i, (i+1) in layers_set))
            else:
    ...
    def on_time(self, current_time, delta):
        for layer in self.layers:
            if layer:
                layer.on_time(current_time, delta)

    def on_message_from_bottom(self, eventobj: Event):
        super().on_message_from_bottom(eventobj)
        for layer in self.layers:
            if layer:
                layer.on_message_from_bottom(eventobj)
